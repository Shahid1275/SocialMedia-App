...
const RecentMessages = () => {
  ...
  return (
    <div className='max-w-xs bg-white mt-4 p-4 min-h-20 rounded-md text-slate-400 shadow text-xs'>
        <h3 className='text-slate-800 font-semibold mb-4'>Recent Messages</h3>
        <div className='flex flex-col max-h-56 overflow-y-scroll no-scrollbar'>
           {messages.map((message,index)=>(
            <Link  to={`/messages/${message.from_user_id._id}`} className='flex items-start gap-2  py-2 hover:bg-slate-100' key={index}>
